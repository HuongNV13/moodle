...
class course_partial_packager extends course_packager {
     ...
     * @param array $cmids List of cmid of selected activities.
    ...
    private function remove_unselected_activities(): void {
        foreach ($this->partialsharingtasks as $task) {
            foreach ($task->get_settings() as $setting) {
                if (in_array($task->get_moduleid(), $this->cmids) && strpos($setting->get_name(), '_included') !== false) {
                    $setting->set_value(1);
                } else {
                    $setting->set_value(0);
                }
