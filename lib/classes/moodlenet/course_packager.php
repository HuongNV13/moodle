...
class course_packager {
    ...
    /**
     * Factory method to load a course packager by course object and userid.
     *
     * @param \stdClass $course The course to package.
     * @param int $userid The ID of the user performing the packaging.
     * @return course_packager
     */
    public static function load_by_instance(
        \stdClass $course,
        int $userid,
    ): self {
        return new self($course, $userid);
    }
     ...
     */
    protected function override_task_setting(
    ...
    /**
     * Package the course identified by courseid into a new stored_file.
     *
     * @return stored_file
     */
    protected function package(): stored_file {
        ...
        if (!isset($result['backup_destination'])) {
            throw new \moodle_exception('Failed to package course.');
        }

        $backupfile = $result['backup_destination'];

        if (!$backupfile->get_contenthash()) {
            throw new \moodle_exception('Failed to package course (invalid file).');
        }
        ...
        $filerecord = [
            ...
            'filepath' => '/',
            'filename' => $this->course->shortname . '_backup.mbz',
        ];
