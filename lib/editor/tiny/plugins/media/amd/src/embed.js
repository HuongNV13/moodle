...
export default class MediaEmbed {
    ...
    async clickHandler(e) {
        ...
        // Fetch preview.
        const fetchPreview = element.closest(Selectors.EMBED.actions.fetchPreview);
        if (fetchPreview) {
            e.preventDefault();
            const hyperlink = document.querySelector(Selectors.EMBED.elements.url).value;

            const request = {
                methodname: 'tiny_media_preview',
                args: {
                    contextid: 1, // Use the system one.
                    content: hyperlink,
                }
            };
            ...
            previewArea.innerHTML = responseObj.content;
            // Trigger the Moodle filters.
            notifyFilterContentUpdated(previewArea);
            window.console.log(hyperlink, responseObj);
        }
    }
