...
import {addIgnoredClassName} from 'tiny_accessibilitychecker/options';

export const setup = async(editor) => {
    const className = 'h5p-placeholder';
    // Register the H5P Formatter to the exceptional class list of Accessibility checker.
    addIgnoredClassName(editor, className);
    ...
    editor.on('SetContent', () => {
        ...
        editor.getBody().querySelectorAll(`.${className}:not([contenteditable])`).forEach((node) => {
