...
export const getSetup = async() => {
    ...
    return (editor) => {
        ...
        if (useContextMenu) {
            ...
            // We only have one option available, so display that as a button.
            editor.ui.registry.addButton(contextMenuName, {
                icon: singleButton,
                tooltip: singleButtonTitle,
                onAction: () => generator.displayContentModal(editor),
            });
        }
    };
};
