...
const getStandardConfig = (target, tinyMCE, options, plugins) => {
    ...
    const config = Object.assign({}, getDefaultConfiguration(), {
        ...
        setup: (editor) => {
            ...
            target.addEventListener(eventTypes.editorVisibilityChanged, (event) => {
                toggleEditorVisibility(target, event.detail.hidden);
            });
 ...
 *
 * @param {Node} target Target element
 * @param {boolean} hidden True if hiding
 */
export const toggleEditorVisibility = (target, hidden) => {
    if (hidden) {
        target.closest('.editor-wrapper').setAttribute('hidden', 'hidden');
    } else {
        target.closest('.editor-wrapper').removeAttribute('hidden', 'hidden');
    }
};
