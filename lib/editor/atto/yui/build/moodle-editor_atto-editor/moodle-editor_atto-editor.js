...
EditorClean.prototype = {
    ...
    _cleanHTML: function(content, deepClean) {
        ...
        var rules = [
            ...
            // Remove insufficient font-size attributes added by Chrome.
            {regex: /<\/?((?:[^>]*?))(style="[^>]*?(font-size:\s(?:[^;">\s]+);?)[^">]*?")(.*?)(?=>)/gi, replace:
                function(match, notuse1, noteuse2, fontsize) {
                    return match.replace(fontsize, '');
                }}
        ];
