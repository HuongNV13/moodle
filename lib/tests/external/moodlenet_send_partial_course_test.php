...
/**
 * External functions test for moodlenet_send_partial_course.
 *
 * @package    core
 * @category   test
 * @copyright  2023 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \core\external\moodlenet_send_partial_course
 */
class moodlenet_send_partial_course_test extends externallib_advanced_testcase {


    /**
     * Test the behaviour of moodlenet_send_partial_course().
     *
     * @covers ::execute
     */
    public function test_moodlenet_send_partial_course() {
        global $CFG;
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $result = moodlenet_send_partial_course::execute($issuer->get('id'), $course->id, 0, $page1->cmid);
        $result = external_api::clean_returnvalue(moodlenet_send_partial_course::execute_returns(), $result);
        ...
        $result = moodlenet_send_partial_course::execute($issuer->get('id'), $course->id, 5, $page1->cmid);
        $result = external_api::clean_returnvalue(moodlenet_send_partial_course::execute_returns(), $result);
        $this->assertFalse($result['status']);
        $this->assertNotEmpty($result['warnings']);
        ...
        $result = moodlenet_send_partial_course::execute($issuer->get('id'), $course->id, 0, random_int(5, 30));
        $result = external_api::clean_returnvalue(moodlenet_send_partial_course::execute_returns(), $result);
        $this->assertFalse($result['status']);
        $this->assertNotEmpty($result['warnings']);
