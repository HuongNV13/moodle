...
class moodlenet_get_share_info_activity_test extends externallib_advanced_testcase {
    ...
    public function test_moodlenet_get_share_info_activity() {
        ...
        $this->assertEquals(get_config('tool_moodlenet', 'defaultmoodlenetname'), $result['server']);
        ...
        $this->assertEquals($randomcmid, $result['warnings']['item']);
        $this->assertEquals('errorgettingactivityinformation', $result['warnings']['warningcode']);
        $this->assertEquals(get_string('invalidcoursemodule', 'error'), $result['warnings']['message']);
    }
}
