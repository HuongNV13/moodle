...
M.form_dndupload.init = function(Y, options) {
    var dnduploadhelper = {
        ...
        drop: function(e) {
            ...
            this.totaluploadfile = files.length;
            if (this.filemanager) {
                var options = {
                    files: files,
        ...
        clear_progress: function() {
            if (this.progressbars === undefined) {
              this.progressbars.progressouter.remove(true);
              delete this.progressbars;
              delete this.totalsuccess;
              delete this.totaluploadfile;
            }
        },
        ...
        update_progress: function(filename, percent) {
            ...
            this.update_total_count(filename);
            if (this.progressbars.progressouter === undefined) {
                var progressouter = this.container.create('<span id="currentuploadname">'+dispfilename+': </span><br><span class="dndupload-progress-outer"><span class="dndupload-progress-inner">&nbsp;</span></span>');
            ...
            this.progressbars.progressinner.setStyle('width', percent + '%');
            this.progressbars.progressinner._node.innerText = percent + '%';
            document.getElementById("currentuploadname").innerHTML = '<b>' + dispfilename + '</b> - ' + this.totalsuccess + '/' + this.totaluploadfile;
        },

        update_total_count: function(filename) {
            if (this.currentuploadname === undefined) {
                this.currentuploadname = filename;
                this.totalsuccess = 1;
            }
            if (this.currentuploadname != filename) {
                this.currentuploadname = filename;
                this.totalsuccess += 1;
            }
        },
    ...
    Y.extend(dnduploader, Y.Base, {
        ...
        initializer: function(params) {
            ...
            this.calculate_total_size(params.files);
        ...
        start_upload: function() {
            this.setactonbuttonvisile(true);
        ...
        uploadfinished: function(lastresult) {
            this.setactonbuttonvisile(false);
        ...
        upload_file: function(file, filename, overwrite) {
            ...
            xhr.upload.addEventListener('progress', function(e) {
                if (e.lengthComputable && self.callbackprogress) {
                    var percentage = self.calculate_overall_percent(filename, e.loaded);
        ...
        setactonbuttonvisile: function(status) {
            require(['jquery'], function($) {
                $(':input[type="submit"]').prop('disabled', status);
            });
        },

        calculate_total_size: function(files) {
            var total_size = 0;
            Array.from(files).forEach(file => {
                total_size += file.size;
            });
            this.total_upload_size = total_size;
        },

        calculate_overall_percent: function(filename, loaded) {
            if (this.currentloadingfile === undefined) {
                this.currentloadingfile = filename;
            }

            if (this.currentloadingfile == filename) {
            ...
            var percentage = Math.round((this.alreadyloaded * 100) / this.total_upload_size);
