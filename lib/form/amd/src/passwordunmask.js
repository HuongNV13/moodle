...
define(['jquery', 'core/templates'], function($, Template) {
    ...
    var PasswordUnmask = function(elementid) {
        ...
        this.inputFieldLabel = $('#' + elementid + '_label');
        this.inputFieldLabelValue = this.inputFieldLabel.text().trim();
    ...
    /**
     * Toggle the labels of the input field for accessibility.
     *
     * @param {Boolean} isEnabled Enable of disable the labels.
     * @method toggleLabels
     */
    PasswordUnmask.prototype.toggleLabels = function(isEnabled) {
        if (isEnabled) {
            // Add the for attribute.
            this.inputFieldLabel.attr('for', this.elementId);
            // Remove the aria-label attribute.
            this.inputField.removeAttr('aria-label');
        } else {
            // Remove the for attribute.
            this.inputFieldLabel.removeAttr('for');
            // Add the aria-label attribute.
            this.inputField.attr('aria-label', this.inputFieldLabelValue);
        }
    };
