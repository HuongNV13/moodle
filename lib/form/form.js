    ...
    Y.extend(dependencyManager, Y.Base, {
        ...
        _hideElement: function(name, hidden) {
            ...
            require(['core_editor/events'], function(editorEvents) {
                editors.each(function(editor) {
                    // Javascript is asynchronous, so we need to wait for the editor to be ready.
                    // Tiny editor will dispatch an event when it is ready.
                    editor.getDOMNode().addEventListener('form:editorUpdated', function() {
                        editorEvents.notifyEditorVisibilityChanged(editor.getDOMNode(), hidden);
                        window.console.log('event dispatched via editorUpdated');
                    });

                    // This will handle the case where the editor is hidden by other elements.
                    editorEvents.notifyEditorVisibilityChanged(editor.getDOMNode(), hidden);
