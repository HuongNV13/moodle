...
const sendToMoodleNet = (issuerId, resourceId, shareFormat) => {
    ...
    if (type === TYPE_ACTIVITY) {
        MoodleNetService.sendActivity(issuerId, resourceId, shareFormat).then(async(data) => {
            const status = data.status;
            const resourceUrl = data.resourceurl;
            return responseFromMoodleNet(status, resourceUrl);
        }).catch(displayException);
    } else if (type === TYPE_COURSE) {
        MoodleNetService.sendCourse(issuerId, resourceId, shareFormat).then(async(data) => {
            const status = data.status;
            const resourceUrl = data.resourceurl;
            return responseFromMoodleNet(status, resourceUrl);
        }).catch(displayException);
...
const registerEventListeners = () => {
    document.addEventListener('click', e => {
        ...
        if (shareAction) {
            ...
            if (type === TYPE_ACTIVITY) {
                MoodleNetService.getActivityInformation(resourceId).then(async(data) => {
                    renderModal(data, shareType).catch(displayException);
                }).catch(displayException);
            } else if (type === TYPE_COURSE) {
                MoodleNetService.getCourseInformation(resourceId).then(async(data) => {
                    renderModal(data, shareType).catch(displayException);
                }).catch(displayException);
            }
        }
