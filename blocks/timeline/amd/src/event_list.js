...
function(
    ...
    var SELECTORS = {
        ...
        EVENT_LIST_CONTENT_DATE: '[data-region="event-list-content-date"]',
    ...
    const loadMoreEvents = root => {
        ...
        eventsPromise.then(data => {
            if (data.calendarEvents.length) {
                const renderPromise = render(data.calendarEvents, midnight);
                const firstEvent = data.calendarEvents[0];
                const lastTimestamp = getLastTimestamp(root);
                const currentTimestamp = firstEvent.timeusermidnight;
                renderPromise.then((html, js) => {
                    html = $(html);

                    if (lastTimestamp == currentTimestamp) {
                        html.find(SELECTORS.EVENT_LIST_CONTENT_DATE).remove();
                    }
