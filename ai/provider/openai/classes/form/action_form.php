...
class action_form extends action_settings_form {
    ...
    public function set_data($default_values): void {
        if (!empty($default_values['modelextraparams'])) {
            $default_values['modelextraparams'] = json_encode(json_decode($default_values['modelextraparams']), JSON_PRETTY_PRINT);
        }
    ...
    protected function add_model_fields(int $modeltype): void {
        ...
        if (!empty($this->actionconfig['model']) && (!array_key_exists($this->actionconfig['model'], $this->get_model_list($modeltype)) || !empty($this->actionconfig['modelextraparams']))) {
            $defaultmodel = 'custom';
        } else {
