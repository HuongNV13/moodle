...
/**
 * Captcha providers manager. Allow to enable/disable captcha providers and jump to settings.
 *
 * @package    core_captcha
 * @copyright  2023 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class manage_captcha_providers_page extends admin_setting {
    ...
    public function output_html($data, $query = ''): string {
        ...
        if (empty($plugins)) {
            return get_string('nocaptchaprovider', 'core_captcha');
        }
        ...
        $table->attributes['class'] = 'managecaptchatable generaltable admintable';
        $table->data = [];

        foreach ($plugins as $plugin) {
            $class = '';
            $actionurl = new moodle_url('/admin/captcha.php', ['sesskey' => sesskey(), 'name' => $plugin->name]);

            if (
                $pluginmanager->get_plugin_info('captcha_' . $plugin->name)->get_status() ===
    ...
    public function is_related($query): bool {
        ...
        $types = core_plugin_manager::instance()->get_plugins_of_type('captcha');
